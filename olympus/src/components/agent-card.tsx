...
function LiveElapsed({ elapsedSeconds }: { elapsedSeconds: number }): JSX.Element {
  ...
  useEffect(() => {
    // Align to wall-clock: compute base at mount, then tick every second
    const base = Date.now();
...
export function AgentCard(props: AgentCardProps): JSX.Element {
  ...
  return (
    <article data-testid="agent-card" className="agent-card" style={{ borderLeftColor: CASTE_COLORS[caste], borderLeftWidth: "3px", borderLeftStyle: "solid" }}>
